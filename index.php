...
if($method == 'POST'){
	...
	switch ($action) {
		case 'weather':
			...
			//Premium API
			$premiumurl=sprintf('http://api.worldweatheronline.com/premium/v1/weather.ashx?key=%s&q=%s&date=%s&format=json', $api_key, $loc, $datum );
			...
			$speech = sprintf("Die Temperatur in %s am %s beträgt %s Grad Celsius.", $json->{'data'}->{'request'}['0']->{'query'}, $newDate, $json->{'data'}->{'current_condition'}['0']->{'temp_C'} );
			break;
		...
		case 'translate.text':
			...
			$speech = sprintf("Der Text \"%s\" heißt übersetzt \"%s\".", $sourceText, $translatedText );
			break;
